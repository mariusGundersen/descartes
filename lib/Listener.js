...
export default class Listener{
  ...
  onCall(tests, timelimit){
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => reject(new Error(`Expected method ${firstTest(tests).expectedIt(stringify)}, actually not called within ${timelimit}ms`)), timelimit);
    });
  }
  called(call){
    if(this.listen == null){
