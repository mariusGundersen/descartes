...
export default class Listener{
  ...
  async awaitCall(target, tests, timelimit){
    const latest = await timeout(this._callQueue.pop(), createTimeoutError(target, tests, timelimit), timelimit);
...
function createTimeoutError(target, tests, timelimit){
  const firstExpected = tests.length === 0
    ? 'to have been called'
    : tests[0].expectedIt(stringify);
  return new Error(`Expected ${target.displayName} ${firstExpected}, actually not called within ${timelimit}ms`);
}
