...
export default class Listener{
  constructor(){
    this.callQueue = new AsyncQueue();
  }
  
  async awaitCall(tests, timelimit){
    const latest = await timeout(this.callQueue.pop(), createTimeoutError(tests, timelimit), timelimit);
    ...
    if (firstFailing == null){
      ...
      throw new Error(`Expected method ${firstFailing.expectedIt(stringify)}, actually ${firstFailing.actually(latest.call, stringify)}`);
    }
  }
